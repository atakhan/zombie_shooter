...
void MovementTestingScene::Update(int *currentSceneIndex) {
    if (continue_) {
        for (auto& system : systems_) {
            if (system == nullptr) {
                continue;
...
void MovementTestingScene::Draw() {
    if (!continue_) {
        return;
    }
...
void MovementTestingScene::GenerateMapEntities(Entity *mapEntity, std::vector<std::vector<int>> spawnMap, std::vector<std::vector<int>> wallsMap) {
